...
async fn main(spawner: Spawner) {
    ...
    let rst = Flex::new(peripherals.GPIO2);
    let te = Flex::new(peripherals.GPIO1);

    let mut co5300 = Co5300::new(spi, te, rst).await.unwrap();
    info!("Hello world!");
    loop {
        Timer::after(Duration::from_secs(1)).await;
        info!("turning on");
        co5300.all_pixels_on().await.unwrap();
        info!("turned on");
        Timer::after(Duration::from_secs(1)).await;
        info!("turning off");
        co5300.all_pixels_off().await.unwrap();
        info!("turned off");
    }
